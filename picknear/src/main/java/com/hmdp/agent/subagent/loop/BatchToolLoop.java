...
/**
 * 批量 + 并行策略：一轮可执行多个独立工具调用。
 * <p>
 * prompt 规则允许"独立且无依赖的工具一次同时调用多个"；轮内执行分两段：
 * ① cb.call（parallelTools=true 时并发）② 长结果压缩 LLM（parallelCompress=true 时并发）——
 * 压缩是耗时大头（~4s/个），并发后 N 个长结果从 N×4s 压到 ~4s。
 * 共享状态（doneSummary/remaining/counters）只在收集完成后的串行段更新，并发段只算纯结果。
 * </p>
 */
@Slf4j
@Component
@ConditionalOnProperty(name = "agent.subtask.tool-loop", havingValue = "batch")
public class BatchToolLoop extends AbstractToolLoop {
    ...
    protected ToolResponseMessage executeRound(AssistantMessage out, SubAgentToolLoopContext ctx,
        ...
        // ═════ 阶段1：cb.call（parallelTools=true 时并发） ═════
        ...
        // CONFIRM：join 后统一重抛（冒泡到 TaskPlanner 建审批，本轮其余结果丢弃）
        for (int i = 0; i < n; i++) {
        ...
        // ═════ 阶段2：长结果压缩（parallelCompress=true 时并发） ═════
        ...
        // ═════ 阶段3：串行组装 + 应用共享状态（线程安全，顺序按 tool_calls 原始顺序） ═════
    ...
    /** 阶段1 结果：raw 或 error（含工具不存在）或 confirm */
    ...
    /**
     * 并发执行一批任务（parallel=false 或单元素时退化为串行）。
     * <p>
     * 观测传播：Micrometer Observation.Scope 是线程局部的，worker 线程默认无观察上下文，
     * 会导致 guard/tool 等子 span 变孤儿。派发前捕获主线程当前 Observation，任务内 makeCurrent 再执行，
     * 让 worker 里 {@code AgentTracer.start} 的父子关系正常挂到当前 trace。
     * </p>
     */
