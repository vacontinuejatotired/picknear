...
- **对话级上下文（2026-09-03 升级）**：记忆回放接通 `agent_message`（模型能记住前面轮次，跨轮问答不再复读/自编）+ **对话级异步压缩**（P2：`recordTurn` 落库后投递独立线程池，后台小模型把旧轮压成运行摘要入库 Redis `Mem` 单 key 视图，回放自动升级为 `[摘要]+增量`；保真断言 dropped 关键数值恒 0 为底线，原值永在 `agent_message` 可回溯）。主请求 token 长期封顶、不随轮数滚雪球。
