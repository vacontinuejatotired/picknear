...
> **版本**: v1.1
> **最后更新**: 2026-09-02
> **上游文档**: [Agent 观测设计](../agent/Agent观测设计.md)
> **用途**: 在主机开发环境落地 M0（配额评估）+ M1（链路打通）的逐条操作指引
>
> **定位注记（2026-08-15）**：本文件是默认观测后端 Langfuse 的操作指南。观测后端已支持可插拔（策略 + 装配器 + 能力驱动，见 [Agent 观测设计](../agent/Agent观测设计.md)）——换后端时本文的链路打通方法不变，只有 endpoint、凭据、专有 header 与配额口径随后端变化；Langfuse 不展示自定义属性，因此依赖 span 名编码。
>
