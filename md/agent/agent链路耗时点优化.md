...
**① 规划时 agent 拿到所有工具的提示词 → 按需加载**

痛点：规划阶段把全部工具提示词拉进 prompt（首拉 0.12s/个走 HTTP，见 §1），工具多了既费 token 又让 LLM 犯蠢选错。

方案（已落地，见 **[《规划工具路由设计》§十一](./规划工具路由设计.md)**）：**意图→工具组两级路由**——`ToolIntentTree` 三层次级意图树（查询/写操作 → 业务类别 → 工具叶子），`TreeCatalogBuilder` 按用户输入剪枝出命中的意图组目录，规划 LLM 只看到相关工具；空命中直接跳过规划（不回归全量）。`feature.tool-routing.enabled` 切换（默认开），legacy 紧凑目录保留。

**② 子 agent 执行时未完成任务占 token / 已完成工具不更新历史摘要 → 每轮重渲染执行 prompt**

痛点：子 agent 多轮工具循环里，任务列表与历史摘要不随执行进度更新，已完成工具的摘要也不回填，未执行任务一直占 token。

方案（已落地）：工具循环每轮用**更新后的计划重渲染执行 prompt**——`remaining` 缩到未执行任务、`doneSummary` 回填已完成工具的压缩摘要（`AbstractToolLoop.renderExecution`），让每轮上下文的「历史执行摘要 / 本轮待执行任务」如实反映进度。配套 `SubAgentToolLoop` 策略（Serial 原按轮 / Batch 批量+并行）见 §4。
