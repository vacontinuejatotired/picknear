...
1. 先读 [文档规范](md/规范/文档规范.md) 和目标模块 `README.md`。
2. 默认不新建文档；优先补进已有文档的对应章节。
3. 状态只使用 `draft`、`current`、`legacy`；不新增完成型交接稿、原始 trace 或无关截图。
4. 新增、删除或重命名当前文档时，同步模块 README 的主题索引和任务路由。
5. 修改后运行 `python scripts/docs/check_docs.py`；检查未通过时不得宣称任务完成。
...
- 修改前端代码请到 `E:\heima\nginx-1.18.0heima\frontend\`（那里有自己的 AGENTS.md 详细说明前端的架构约定和代码规范）
